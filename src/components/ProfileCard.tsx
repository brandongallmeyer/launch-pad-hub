...
const ProfileCard = ({
  ...
  return <div className="w-full max-w-7xl mx-auto px-4 relative">
      ...
      <div className="relative z-10 text-center mb-16">
        ...
        {/* About Me Button */}
        {!showBio && <Button onClick={startTypingAnimation} className="relative overflow-hidden bg-gradient-real-estate hover:bg-gradient-real-estate text-background shadow-button hover:shadow-neon-green transition-all duration-300 hover:scale-105 border border-accent-green/30 px-6 py-3 mb-8 text-lg font-semibold">
            <Play className="w-5 h-5 mr-2" />
            About Me
          </Button>}
        
        {/* Animated Bio */}
        {showBio && <div className="text-lg text-muted-foreground max-w-3xl mx-auto leading-relaxed mb-8">
            <p>{displayedBio}</p>
            {isTyping && <span className="animate-pulse">|</span>}
            {!isTyping && <div className="flex justify-center mt-4">
                <Button onClick={() => {
            setShowBio(false);
            setDisplayedBio("");
            setIsTyping(false);
          }} variant="ghost" size="sm" className="text-muted-foreground hover:text-foreground transition-colors duration-200">
      ...
      <div className="relative z-10 grid lg:grid-cols-2 gap-8 mb-16">
        ...
        <div className="flip-container group cursor-pointer h-80 md:h-96" onMouseEnter={() => window.innerWidth >= 768 && setRealEstateFlipped(true)} onMouseLeave={() => window.innerWidth >= 768 && setRealEstateFlipped(false)} onClick={() => setRealEstateFlipped(!realEstateFlipped)}>
          ...
          transform: realEstateFlipped ? 'rotateY(180deg)' : 'rotateY(0deg)'
            ...
            backfaceVisibility: 'hidden'
              ...
              <div className="relative p-6 md:p-8 text-center h-full flex flex-col justify-center">
                ...
                <p className="text-muted-foreground mb-6 md:mb-8 leading-relaxed text-base md:text-lg">Whether you're a first-time buyer or experienced investor, I'm here to make buying and selling real estate fun and stress-free. I'll guide you through every step because getting it right starts with the right agent, and you're already halfway there!</p>
        ...
        <div className="flip-container group cursor-pointer h-80 md:h-96" onMouseEnter={() => window.innerWidth >= 768 && setHorizonFlipped(true)} onMouseLeave={() => window.innerWidth >= 768 && setHorizonFlipped(false)} onClick={() => setHorizonFlipped(!horizonFlipped)}>
          <div className={`flip-inner h-full transition-transform duration-600`} style={{
          transformStyle: 'preserve-3d',
      ...
      <ContactDialog isOpen={isContactDialogOpen} onClose={() => setIsContactDialogOpen(false)} phone={phone} email={email} />
    </div>;
};
